...
const CategoryForm = ({initialData, courseId, options}) => {
  ...
  const onSubmit = async(values) => {
    ...
    <div className='mt-6 bg-slate-100 rounded-md dark:bg-gray-800'>
        ...
        {
            !isEditing && (
                <p className={cn("text-sm mt-2 dark:text-gray-300 px-3 pb-3", !initialData?. category && "italic text-gray-400")}>
                    {initialData?.category || "Category not set for this course"}</p>
            )}
            {
                isEditing && (
                    <Form {...form}>
