...
const DurationForm = ({initialData, courseId}) => {
  ...
  const onSubmit = async(values) => {
    ...
    <div className='mt-6 bg-slate-100 rounded-md dark:bg-gray-800'>
        ...
        {
            !isEditing && (
                <p className={cn("text-sm mt-2 dark:text-gray-300 px-3 pb-3", !initialData?. duration && "italic text-gray-400")}>
                    {formatDuration(initialData?.duration) || "Duration set for this course"}</p>
            )}
            {
                isEditing && (
                    <Form {...form}>
                        <form onSubmit={form.handleSubmit(onSubmit)} className='p-3'>
                            <FormField
                                ...
                                render={({field}) => (
                                    <FormItem>
                                        <FormControl>
                                            <Input disabled={isSubmitting} placeholder="e.g 6 weeks"  className="bg-white" {...field}/>
                                        </FormControl>
                                        <FormDescription>Put data only in number of weeks</FormDescription>
                                        <FormMessage/>
                                    </FormItem>
                                )}/>
